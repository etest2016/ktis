...
<% 	
	...
	src_pub_comp1 = new String(request.getParameter("src_pub_comp1").getBytes("8859_1"),"EUC-KR");
	...
	qs = new String(request.getParameter("qs").getBytes("8859_1"),"EUC-KR");
	...
	if (posStart.equals("0")){
		...
		try {
			totalCount = String.valueOf(QSearch.getCnt(bean, id_course, chk_userid));
		} catch(Exception ex) {
	...
	try {
		rst = QSearch.getSearchBeans(bean, id_course, chk_userid, Integer.parseInt(posStart), Integer.parseInt(count));
	} catch(Exception ex) {
		out.println(ComLib.getExceptionMsg(ex, "back"));
    ...
    if(rst == null) {
		...
		{			
			...
			out.println("<cell>" + StringEscapeUtils.escapeXml(rst[i].getId_subject()) + "</cell>");
			out.println("<cell>" + StringEscapeUtils.escapeXml(rst[i].getId_chapter()) + "</cell>");
			out.println("</row>");
		}
