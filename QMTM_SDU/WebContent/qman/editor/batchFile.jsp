...
<%
	...
	if (id_q_subject.length() == 0 || id_q_chapter.length() == 0 || qtype.length() == 0) {
...
{	
	...
	if(caNotChk == "N") {
		alert(caNotStr+"문제 정답이 잘못되었습니다.\n\n작업결과 화면에서 오류 문제를 확인해서 수정한 후\n\n패턴적용을 다시 진행하시기 바랍니다.");
		return;
	}
...
// 정답 누락 체크
function ca_check(num, ca, cacount, qtype) {
	
	var ArrCa = new Array();
	ca = ca.replaceAll(" ","");

	ca = htmlDel(ca.replaceAll("&NBSP;",""));

	if(ca.length == 0) {
		alert((num+1)+"번 문제 정답이 누락되었습니다.\n\n작업파일에서 해당 문제 정답을 입력하시고\n\n패턴적용을 진행해주시기 바랍니다.");
	...
	if(qtype < 4) { 
		if(cacount == 1) {
			if(ca.length > 1) {
				...
				alert((num+1)+"번 문제 정답이 잘못된 형식입니다.\n\n작업파일에서 해당 문제 정답에 숫자만 다시 입력하시고\n\n패턴적용을 진행해주시기 바랍니다.");			
...
{		
	...
	document.twe13.GetLocalFiles; // 이진파일 경로 가져오기
	...
	if(non_qs == "N") {	
		...
		if((Qchk-1) != qcounts) {
			alert("입력하신 문제수와 원본문서의 문제수가 맞지 않습니다.\n\n확인 후 다시 진행해주시기 바랍니다.");
			Show_LayerProgressBar(false);
			return;
		}
	}
		...
		for(var a=0; a<qcounts; a++) {
			...
			if(arrQ[a].indexOf(e1+e2+e3.substring(0,1)+e4) == -1) {	// 단답형인지, 논술형인지 체크한다.		
				...
				if(arrQ[a].indexOf(ca_ptn) == -1) { // 정답이 없으면 논술형임.
					...
					if(ca_check(a, arrCa[a], arrCacount[a], arrQtype[a])) {
						arrCachk[a] = "Y";					
					} else {
						arrCachk[a] = "N";
					}
			...
			if(arrQ[a].indexOf(e1+e2+e3.substring(2,3)+e4) == -1) { // 선다형 3번 보기가 없을 경우(OX형문제)
				...
				arrQ[a] = arrQ[a].substring(arrQ[a].lastIndexOf(q1+q2+arrMunje_strs[a]+q4),arrQ[a].indexOf(ca_ptn)) + text_convert(arrQ[a].substring(arrQ[a].lastIndexOf(ca_ptn)));
...
{	
	...
	document.all.workdivNot.style.display = "none";
	...
	strArea += "<a href='javascript:workResult("+(qcounts-1)+")'>[ 마지막으로 ]</a>&nbsp;&nbsp;&nbsp;<a href='javascript:workResultAll()'>[ 전체문제보기 ]</a>&nbsp;&nbsp;<a href='javascript:workResultNot()'>[ 정답오류보기 ]</a></td>";	
	strArea += "<tr>";
	strArea += "</table>";
	strArea += "</td>";	
	strArea += "</tr>";
	...
	if(arrCachk[i] == "N") {
		strArea += "<tr bgcolor='#FFFFFF' height='30'>";
		strArea += "<td align='center' bgcolor='red'><b>정답오류</b></td>";
		strArea += "<td>"+htmlDel(arrCa[i]).replaceAll("&NBSP;","")+"<BR><font color=green><b>(정답에 오류가 발생했습니다. 작업파일에서 수정 후 패턴설정을 진행해주세요.)</b></font></td>";
		strArea += "</tr>";
	} else {
		strArea += "<tr bgcolor='#FFFFFF' height='30'>";
...
function workResultAll() 
{
	...
	for(var k=0; k<qcounts; k++) {		
		eval("form.qs" + k + ".value = removeBR(arrQs[k]).replaceAll('&NBSP;','');");
		if(arrExcount[k] == 2) {
			eval("form.ex1" + k + ".value = removeBR(arrEx1[k]).replaceAll('&NBSP;','');");
			eval("form.ex2" + k + ".value = removeBR(arrEx2[k]).replaceAll('&NBSP;','');");
		} else if(arrExcount[k] == 3) {
			eval("form.ex1" + k + ".value = removeBR(arrEx1[k]).replaceAll('&NBSP;','');");
			eval("form.ex2" + k + ".value = removeBR(arrEx2[k]).replaceAll('&NBSP;','');");
			eval("form.ex3" + k + ".value = removeBR(arrEx3[k]).replaceAll('&NBSP;','');");
		} else if(arrExcount[k] == 4) {
			eval("form.ex1" + k + ".value = removeBR(arrEx1[k]).replaceAll('&NBSP;','');");
			eval("form.ex2" + k + ".value = removeBR(arrEx2[k]).replaceAll('&NBSP;','');");
			eval("form.ex3" + k + ".value = removeBR(arrEx3[k]).replaceAll('&NBSP;','');");
			eval("form.ex4" + k + ".value = removeBR(arrEx4[k]).replaceAll('&NBSP;','');");
		} else if(arrExcount[k] == 5) {
			eval("form.ex1" + k + ".value = removeBR(arrEx1[k]).replaceAll('&NBSP;','');");
			eval("form.ex2" + k + ".value = removeBR(arrEx2[k]).replaceAll('&NBSP;','');");
			eval("form.ex3" + k + ".value = removeBR(arrEx3[k]).replaceAll('&NBSP;','');");
			eval("form.ex4" + k + ".value = removeBR(arrEx4[k]).replaceAll('&NBSP;','');");
			eval("form.ex5" + k + ".value = removeBR(arrEx5[k]).replaceAll('&NBSP;','');");
		}
		eval("form.ca" + k + ".value = htmlDel(arrCa[k]).replaceAll('&NBSP;','');");
		eval("form.explain" + k + ".value = removeBR(arrExplain[k]).replaceAll('&NBSP;','');");

		eval("form.hint" + k + ".value = removeBR(arrHint[k]).replaceAll('&NBSP;','');");
		eval("form.difficulty" + k + ".value = removeBR(arrDifficulty[k]).replaceAll('&NBSP;','');");

		eval("form.qtypes" + k + ".value = qtypes;");
		eval("form.excount" + k + ".value = arrExcount[k];");
		eval("form.cacount" + k + ".value = arrCacount[k];");
	}

	strArea += "<table border='0' width='100%'>";
	strArea += "<tr>";
	strArea += "<td><a href='javascript:workResult(0)'>[ 한문제씩 보기 ]</a>&nbsp;&nbsp;<a href='javascript:workResultAll()'>[ 전체문제보기 ]</a>&nbsp;&nbsp;<a href='javascript:workResultNot()'>[ 정답오류보기 ]</a></td>";
	strArea += "</tr>";
	strArea += "</table>";
	...
	strArea += "<tr bgcolor='#FFFFFF' height='30'>";
	strArea += "<td align='center' valign='top' width='20%' bgcolor='#EEEEEE'><font color=red><b>"+(i+1)+".</b></font></td>";
	strArea += "<td valign='top'>"+removeBR(arrQs[i]).replaceAll("&NBSP;","")+"</td>";
	strArea += "</tr>";
	if(arrExcount[i] == 2) {
		strArea += "<tr bgcolor='#FFFFFF' height='30'>";
		strArea += "<td align='center' bgcolor='#EEEEEE'>①</td>";
...
function workResultNot() 
{
	document.all.workdiv.style.display = "none";
	document.all.workdivAll.style.display = "none";
	document.all.workdivNot.style.display = "block";
	...
	strArea += "</table>";	
		...
		strArea += "<table border='0' width='100%'>";
		strArea += "<tr><td valign='center'>";	
		strArea += "<table border='0' align='left' valign='top' width='610'>";
		strArea += "<tr height='30'>";
		strArea += "<td colspan='2' align='center'>정답오류 문제가 없습니다.</td>";
		strArea += "</tr>";	
		strArea += "</table>";
		strArea += "</td>";	
		strArea += "</tr>";
	...
	document.all.workdivNot.innerHTML = strArea;
...
function pattern_extract(a, b, c, d, e, f) {
	...
	$.posterPopup('pattern_extract.jsp?a='+a+'&b='+b+'&c='+c+'&d='+d+'&e='+e+'&f='+f,'pattern_extract','width=500, height=250, scrollbars=no, top='+(screen.height-250)/2+', left='+(screen.width-500)/2);
}
...
function diff_extract() {
	...
	$.posterPopup('diff_extract.jsp?a='+a+'&b='+b+'&c='+c+'&d='+d+'&e='+e+'&f='+f,'diff_extract','width=200, height=400, scrollbars=no, top='+(screen.height-400)/2+', left='+(screen.width-200)/2);
}
...
<body onLoad="movieLayout('div_0');" id="popup2">
	...
	<div id="contents" style="height: 420px;">
		...
		<% for(int i = 0; i < 100; i++) { %>
			<input type=hidden name="qs<%=i%>">
			<input type=hidden name="ex1<%=i%>">
			<input type=hidden name="ex2<%=i%>">
			<input type=hidden name="ex3<%=i%>">
			<input type=hidden name="ex4<%=i%>">
			<input type=hidden name="ex5<%=i%>">
			<input type=hidden name="ca<%=i%>">
			<input type=hidden name="explain<%=i%>">
			<input type=hidden name="hint<%=i%>">
			<input type=hidden name="difficulty<%=i%>">
			<input type=hidden name="qtypes<%=i%>">
			<input type=hidden name="excount<%=i%>">
			<input type=hidden name="cacount<%=i%>">
		<% } %>
		...
		<table border="0" width="95%" cellspacing="0" cellpadding="0">
			<tr>
				<td>* 문제수 : <input type="text" name="qcounts" size="3"> 문제
					<a href="<%=ComLib.getConfig("adminurl")%>/q_sample.zip"><b>[문제일괄등록샘플파일]</b></a>
					&nbsp;&nbsp;
					<a href="<%=ComLib.getConfig("adminurl")%>/q_guide.zip"><b>[문제저작가이드]</b></a>
				</td>
				<td align="right">
		...
		<div style="display:none;" id="div_0">
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
				<tr>
					<td align="left">&nbsp;&nbsp;▶ 작업파일선택<!--&nbsp;<font color=red><b>텍스트문서로 일괄등록(일괄등록 파일에 이미지가 없을경우 체크)</b></font>--></td>
				</tr>
				<tr>
					<td valign="top">
		...
		<div style="display:none;" id="div_1">
			...
			<div style="display:none;" id="non_chk">
				...
				<tr>
					<td><input type="checkbox" name="non_q" value="Y"> <font color=green><b>정답패턴에 오류가 발생한 문제가 모두 논술형일경우 체크해주시기 바랍니다.</b></font></td>			
				</tr>
			</table>	
			</div>
		...
		<div style="display:none;" id="div_4">
			...
			<table border="0">
				<tr>
					<td width="100%" >
					<div id="workdivNot" style="width: 100%;"></div>
					</td>			
				</tr>
			</table>
