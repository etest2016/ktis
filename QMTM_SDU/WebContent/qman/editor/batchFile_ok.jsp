...
<%@ page import="java.io.*, java.util.*, com.tagfree.util.*, org.w3c.tidy.*, org.w3c.dom.*, qmtm.*, qmtm.common.*, qmtm.qman.editor.*, qmtm.qman.*, qmtm.admin.etc.EnvUtil, qmtm.admin.etc.EnvUtilBean, org.apache.commons.net.ftp.*" %>
...
<%
		...
		strMimeValue = strMimeValue.replace("<BR>","&ltBR&gt");
				
		//외부개체가 포함되었을경우
		if(strMimeValue.replace("\"","").indexOf("<?xml:namespace prefix = v ns = urn:schemas-microsoft-com:vml />") != -1) {			
			
			pre_str = strMimeValue.substring(0,strMimeValue.replace("\"","").indexOf("<?xml:namespace prefix = v ns = urn:schemas-microsoft-com:vml />"));

			strMimeValue = strMimeValue.substring(strMimeValue.replace("\"","").indexOf("<?xml:namespace prefix = v ns = urn:schemas-microsoft-com:vml />")); 
		...
		strMimeValue = pre_str + strMimeValue;		

		strMimeValue = strMimeValue.replace("&ltBR&gt","<BR>");
		strMimeValue = strMimeValue.replaceAll("&ltBR&","<BR>");
		...
		String[] arrQ_ks = new String[qcount];
		String[] arrQ_to = new String[qcount];
		...
		if(q3.equals("[0-9]")) {
			...
			if(arrQ[a].indexOf(e1+e2+e3.substring(2,3)+e4) == -1) { // 선다형 3번 보기가 없을 경우(OX형문제)
				...
				arrQ[a] = arrQ[a].substring(arrQ[a].lastIndexOf(q1+q2+arrMunje_result[a]+q4),arrQ[a].indexOf(ca_ptn)) + ComLib.text_convert(arrQ[a].substring(arrQ[a].lastIndexOf(ca_ptn)));
	...
	for(int k=0; k < qcount; k++) {
		...
		arrDifficulty[k] = ComLib.removeNBSP(ComLib.removeBR2(ComLib.nullChk(arrDifficulty[k])));
		...
		bean.setQ(ComLib.nullChk(arrQs[k]));
		bean.setEx1(ComLib.removeBR(ComLib.nullChk(arrEx1[k])));
		bean.setEx2(ComLib.removeBR(ComLib.nullChk(arrEx2[k])));
		bean.setEx3(ComLib.removeBR(ComLib.nullChk(arrEx3[k])));
		bean.setEx4(ComLib.removeBR(ComLib.nullChk(arrEx4[k])));
		bean.setEx5(ComLib.removeBR(ComLib.nullChk(arrEx5[k])));
		...
		arrCa[k] = ComLib.htmlDel(arrCa[k]);
	...
	bigos.append(qcounts);
	bigos.append("문제 일괄등록");
...
%>
<script language="javascript">
	alert("2.문제 일괄등록 작업중에 문제가 발생했습니다.\n\n이미지 파일이 원본문서에 포함되어 있을경우\n\n파일 Size가 클경우 업로드가 되지 않는경우가 있습니다.\n\n이미지 파일을 편집한 후 다시 진행해주시기 바랍니다.\n\n\n계속해서 문제가 발생할 경우 관리자에게 문의하시기 바랍니다.");
	window.opener.location.reload();
	window.close();
</script>
<%
