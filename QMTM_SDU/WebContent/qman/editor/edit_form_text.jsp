...
//   모 듈 명 : 개별문제 텍스트 수정
//   설    명 : 개별문제 텍스트 수정화면
//   테 이 블 : r_difficulty, r_q_use, q_standard_a, q_standard_b, q, q_ref
...
<body onLoad="inits();">
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
		<tr>
			<td valign="top">
				<div style="height:32px; background-image: url(../../images/editor_newq.gif); background-repeat: no-repeat; padding: 6px 0px 0px 90px; font: bold 16px dotum; color: #000000;"> <%=id_q%>&nbsp;-&nbsp;<%=valid_types%>&nbsp;(<%=make_cnt%>회 출제)</div>
				<table border="0">
					<tr>
			...
			<td width="500" align="left" valign="top" bgcolor="ffffff">
				...
				<div id="id_q">
					...
					<table border="0" width="97%" cellspacing="0" cellpadding="0" align="center">
						<tr>
							<td height=25 valign="top"><input type="checkbox" name="yn_ex_set" <% if(yn_ex_set.equals("Y")) { %> checked <% } %> value="Y"> <B>문제출제시 보기섞기 금지</B></td>
						</tr>
					</table>					
					<% } %>
								...
								<table width="100%" border="0">
									...
									<td width="520" valign="top" colspan="2">
										...
										<table width="100%" border="0" style="display:none;" id="id_correct<%=i+1%>" cellpadding=0 cellspacing=0>
											...
											<tr>
												<td colspan="2"><textarea cols="65" rows="4">※ 설정한 정답갯수대로 정답을 입력하세요.
...
예를 들어 정답이 '인간' 이라고 하면 '사람' 이라는 단어도 정답으로 인정된다고 생각되면 모두 입력하세요.</textarea></td>
											...
											<tr height="25">
												<td colspan="2">정답 입력 텍스트 박스 크기 : <input type="text" class="input" name="ans_size<%=i+1%>" size="4" value="<%=ArrBoxSize[i]%>"></td>
											</tr>						
											<tr height="25">
												<td colspan="2">앞 메시지 : <input type="text" class="input" name="front_msg<%=i+1%>" size="43" value="<%=ArrFrontMsg[i]%>"></td>
											</tr>
											<tr height="25">
												<td colspan="2">뒷 메시지 : <input type="text" class="input" name="back_msg<%=i+1%>" size="43" value="<%=ArrBackMsg[i]%>"></td>
											</tr>
										</table>
										<% } %>
									</td>
...
예)  정답이 '단답형 정답 입력' 이고, '띄어쓰기 구분 있음' 으로 할 경우 '단답형정답 입력' 으로 답안을 제출하면 틀린 답안으로 채점이 됩니다.			
					...
					<% } else if(qtype.equals("5")) { %>
					<tr bgcolor="#FFFFFF">
						<td width="525" valign="top" colspan="2"><input type="checkbox" name="yn_practice" <% if(yn_practice.equals("Y")) { %> checked <% } %> value="Y"> <B>실기형 문제로 출제</B><BR><BR><B>'실기형 문제로 출제' 에 체크하면 응시자 시험지에<BR>답안 입력하는 박스가 나타나지 않는 형태로 출제됩니다.</B></td>
					</tr>
					<% } %>
				</table>
				</div>
			...
			<div style="display:;" id="id_refbody">
				...
				<table border="0" width="95%" cellspacing="0" cellpadding="0" align="center">
					<tr height="30">
						<td bgcolor="FFFFFF" align="left"><input type="button" value="지문초기화" onClick="q_refs_init();" class="form2">&nbsp;&nbsp;<input type="button" value="기존지문선택"  class="form2" onClick="q_refs();"></td>			
					</tr>
					<tr height="25">				
			...
			<div style="display:none;" id="id_prints">
				...
				<table border="0" border="1" width="95%" cellspacing="0" cellpadding="3" id="tableD" align="center">
					...
					<tr>
						<td bgcolor="FFFFFF" align="right" width="22%" id="left">소분류</td>
