...
<%
	...
	try {
		total_count = Score_DanAns.userAnsCount(id_exam, id_q, kwd_ans, "N");
	} catch(Exception ex) {
	...
	try {
		rst = Score_DanAns.userAns(page_scale, lists, id_exam, id_q, kwd_ans, "N");
	} catch(Exception ex) {
...
<script language="JavaScript">
	...
	function user_score(answers) {
		$.posterPopup("answer_user_score.jsp?id_exam=<%=id_exam%>&id_q=<%=id_q%>&answers="+answers,"u_score","width=400, height=300, scrollbars=yes");
	}
</script>
</head>
...
<div style="width: 100%; margin: 30px 40px 40px 40px;">
	...
	<hr>
	<img src="../images/sub2_webscore3.gif"> <font color=red size=3><b>(채점 미완료자 목록)</b></font>
	...
	<TABLE width="100%" cellpadding="0" cellspacing="0" border="0">
		<TR>
			...
			<TD align="right">총 <font color='red'><%=total_count%></font> 개의 오답리스트가 존재합니다.</TD>
		</TR>
	</TABLE>
	</form>
	...
	%>
		<tr>
			...
			<td><%= answer2 %></td>
			<% } %>
			<td><a href="javascript:" onClick="user_score('<%=answer%>');"><%= rst[i].getCnt() %> 명</a></td>
		</tr> 
