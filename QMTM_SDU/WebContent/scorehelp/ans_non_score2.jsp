...
<script language="JavaScript">
	...
	function equal_values() {
		...
		if (frm.equal_value.value == "" && frm.k_equal_value.value == "" && frm.ans_len_value.value == "")
		{
			...
			return;
		} else if (!(frm.k_equal_value.value >= '0' && frm.k_equal_value.value <= '9'))
		{
			alert("유사율은 숫자만 입력가능합니다.");
			frm.k_equal_value.value = "";
			frm.k_equal_value.focus();
		} else if (!(frm.equal_value.value >= '0' && frm.equal_value.value <= '9'))
		{
			alert("유사율은 숫자만 입력가능합니다.");
			frm.equal_value.value = "";
			frm.equal_value.focus();
		} else if (!(frm.ans_len_value.value >= '0' && frm.ans_len_value.value <= '9'))
		{
			alert("답안길이 입력란은 숫자만 입력가능합니다.");
			frm.ans_len_value.value = "";
			frm.ans_len_value.focus();
...
<BODY Style="margin:0px;">
	...
	<!-- 타이틀 디자인 -->
	<div style="width: 100%; height: 70px; background-image: url(../images/bg_mark_top.gif); text-align: right;"><img src="../images/bt_re.gif" onclick="location.href='ans_non_score2.jsp?id_exam=<%=id_exam%>&id_q=<%=id_q%>';" style="cursor: pointer;"><img src="../images/bt_out.gif" style="cursor: pointer;" onclick="javascript:window.close();"></div>
	<!-- 단답형 문제 웹채점 -->
		...
		<tr>
			<td align="center" style="padding-left: 30px; padding-right: 30px;">
				...
				<%
					...
					//out.println(k_equal_value+"<br>");
					//out.println(equal_value+"<br>");
					//out.println(ans_len_value);

					//페이징 처리를 위해 전체 레코드를 구해서 전체 페이지를 구한다.
					try {
						TotRecord = Score_AnsNon.getCount(id_exam, id_q, k_equal_value, equal_value, ans_len_value, "Y");
					...
					try {
						...
						beans = Score_AnsNon.getBeans(id_exam, id_q, k_equal_value, equal_value, ans_len_value, "Y", iPage, pgSize);
					} catch(Exception ex) {
						out.println(ex.getMessage());
						if(true) return;
						//response.sendRedirect("/error/db_error.jsp?err_msg="+ex.getMessage());
				...
				<div class="title">채점완료자 리스트</div>

				<table border='0' width='100%'>
					<tr>
					<!--<form name="form2" method="post" action="ans_non_score2.jsp">
					<input type="hidden" name="id_exam" value="<%=id_exam%>">
					<input type="hidden" name="id_q" value="<%=id_q%>">-->
						<td align="left" colspan="2"><div style="float: left; padding-top: 3px;">&nbsp;&nbsp;총 <font color='red'><b><%=TotRecord%></b></font> 명&nbsp;&nbsp;&nbsp;<% if(beans != null){ //if(beans[0].getBasic_ans_len()!=0){ %><b>모범답안길이 : <font color=red><%=beans[0].getBasic_ans_len()%>Len</font></b><%}//}%></div><div style="float: left;">&nbsp;&nbsp;&nbsp;<a href="ans_non_score.jsp?id_exam=<%=id_exam%>&id_q=<%=id_q%>"><img src="../images/bt8_c.gif"></a></div></td>
					</tr>
					<!--<tr>
				...
				name="ans_len_value" size="3" class="form2" value="<%=ans_len_value%>">&nbsp;Len ↑&nbsp;</B></td>
				...
				<table border='0' cellspacing='0' cellpadding='3' width='100%' id="tableD" style="margin-top: 5px;">
					...
					<input type="hidden" name="id_exam" value="<%=id_exam%>">
					<input type="hidden" name="id_q" value="<%=id_q%>">
						<td width="5%"><B>선택 <INPUT type=checkbox onclick=chkBox(this.checked) class="form3"></B></td>  
						<td width="4%"><B>NO</B></td>  
						<td width="15%"><B>응시자</B></td>
						<td width="14%"><B>키워드 유사율</B></td>    
						<td width="14%"><B>모범답안 유사율</B></td>    
						<td width="15%"><B>응시자 답안길이</B></td>    
						<td width="10%"><B>득점</B></td>    
						<td ><B>개별채점</B></td>    
					</tr>
					...
					<tr bgcolor="#FFFFFF" align="center" height="30">  
						<td colspan="8" class="blank"><B>채점완료자가 없습니다.</B></td>
					</tr>
				...
				<table border='0' width='100%' style="margin-top: 5px;">
					<tr>
						...
						<!--<a href="javascript:" onclick="chkBox()">전체선택</a> || <a href="javascript:" onclick="revBox()">선택해제</a>&nbsp;&nbsp;&nbsp;-->선택응시자 일괄채점 : <input type="text" name="tot_score" size="3" class="form2"> 점 &nbsp;&nbsp;
						</div>
						<div style="float: left;">
						&nbsp;&nbsp;<img src="../images/bt3_true.gif" onClick="ans_tot_score(1, <%=allotting%>);" style="cursor: pointer;">&nbsp;&nbsp;<img src="../images/bt3_false.gif" onClick="ans_tot_score(0, <%=allotting%>);" style="cursor: pointer;">&nbsp;&nbsp;<img src="../images/bt3_mark.gif"  onClick="ans_tot_score(<%=allotting%>);" style="cursor: pointer;"></div></td>
						</form>
						<td align="right">
							...
							<table cellpadding="2" cellspacing="0" border="0">
								<tr>
									...
									%>
										[<a href="ans_non_score2.jsp?<%=Tar%>">이 전</a>]
									<% 
										}
										//이전그룹 끝---------------
									...
									%>
										[<a href="ans_non_score2.jsp?<%=Tar%>"><%=intI%></a>]
									<%
											LCount = LCount -1;
											if (LCount == 0) {
												break;
									...
									%>
										[<a href="ans_non_score2.jsp?<%=Tar%>">다음</a>]
									<% 
										//다음그룹 끝---------------
