...
<HTML>
 <HEAD>
  ...
  <script type="text/JavaScript">
    ...
    function insert(jucha) {
		$.posterPopup("exam/exam_write.jsp?id_course=<%=id_course%>","insert","width=750, height=680, scrollbars=no, top="+(screen.height-680)/2+", left="+(screen.width-750)/2);
    }
	...
	%>
			<tr id="td" align="center">
				...
				<% if(rst[i].getId_exam().equals("")) { %>
					...
					<td><%if(pt_exam_edit.equals("Y")) { %><input type="button" value="퀴즈평가등록" onclick="simpleInsert(<%=rst[i].getJucha()%>);" class="form4">&nbsp;&nbsp;<input type="button" value="퀴즈평가 문제은행방식등록" onclick="Insert(<%=rst[i].getJucha()%>);" class="form4"><% }else { %><input type="button" value="퀴즈평가화등록하기" onclick="alert('퀴즈평가 등록 권한이 없습니다.');" class="form4">&nbsp;&nbsp;<input type="button" value="퀴즈평가 문제은행방식등록" onclick="alert('퀴즈평가 등록 권한이 없습니다.');" class="form4"><% } %></td>
				<% } else { %>
