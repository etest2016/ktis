...
<%
	...
	try {
		paper_cnt = ExamUtil.getPaperCnt(id_exam);
	} catch(Exception ex) {
		out.println(ComLib.getExceptionMsg(ex, "back"));
...
<script language="JavaScript">
	...
	function exam_make() {
		var ans_cnt = <%=ans_cnt%>;
		var paper_cnt = <%=paper_cnt%>;

		if(paper_cnt > 0) {
			alert("이미 시험지가 생성되었습니다.\n\n시험지 삭제 후 진행해주시기 바랍니다.");
			return;
		} else {
			if(ans_cnt > 0) {
				...
				$.posterPopup("exam_make_write.jsp?id_exam=<%=id_exam%>","exam_make","width=970, height=700, scrollbars=yes, top=0, left=0");
			}
		}
	...
	function paper_guide() {
		$.posterPopup("paper_guide_list.jsp?id_exam=<%=id_exam%>","paper_guide","width=650, height=500, scrollbars=yes, top=0, left=0");
    }

	function paper_option() {
		$.posterPopup("paper_option.jsp?id_exam=<%=id_exam%>","paper_option","width=700, height=600, scrollbars=yes, top=0, left=0");
    }

	function paper_preview() {
		$.posterPopup("paper_preview.jsp?id_exam=<%=id_exam%>&nr_set=<%=nr_set%>","paper_preview","width=400, height=250, scrollbars=yes, top=0, left=0");
	}

	function chapter_score() {
		$.posterPopup("chapter_score_list.jsp?id_exam=<%=id_exam%>","chapter_score","width=700, height=350, scrollbars=yes, top=0, left=0");
	}
		...
		<TR>
			...
			<TD style="background-image: url(../../images/bg_mpaper_r.gif); background-repeat: repeat-x;" valign="top">
				<div style="float: left; padding-top: 9px; padding-left: 20px;">
					<a href="javascript:exam_make();">시험지만들기</a>&nbsp;&nbsp;&nbsp;<a href="javascript:exam_delete();">시험지삭제</a>&nbsp;&nbsp;&nbsp;<a href="javascript:paper_option();">출제옵션</a><!--&nbsp;&nbsp;&nbsp;<a href="javascript:paper_option();">문제교체</a>-->&nbsp;&nbsp;&nbsp;<a href="javascript:paper_preview();">미리보기</a><!--&nbsp;&nbsp;&nbsp;<a href="javascript:paper_preview();">내보내기</a>-->
				</div>
				<div style="float: right; margin-right: 15px;"><img src="../../images/bt_mpaper_out.gif" onclick="goout_Click();" style="cursor: pointer;"></div>
			</TD>
		</TR>
	</TABLE> 
</BODY>
