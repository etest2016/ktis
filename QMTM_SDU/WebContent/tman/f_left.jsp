...
//   모 듈 명 : 시험관리 좌측 프레임
//   설    명 : 시험관리 좌측 프레임
//   테 이 블 : c_category, c_midcategory, c_course 
...
<%
	...
	if(userid.equals("qmtm") || usergrade.equals("M")) {		
		...
		try {
			rst = TreeList.getAdmCosBeans(arrTerm_id[0], arrTerm_id[1], aligns);		
		} catch(Exception ex) {
		...
		try {
			rst = TreeList.getCosBeans(id_group, arrTerm_id[0], arrTerm_id[1], aligns);
		} catch(Exception ex) {
			out.println(ComLib.getExceptionMsg(ex, "back"));
	...
	if(!id_subject.equals("")) {
		...
		if(userid.equals("qmtm") || usergrade.equals("M")) {		
			...
			try {
				rst2 = TreeList.getAdmExamBeans(arrTerm_id[0], arrTerm_id[1], aligns, id_subject);		
			} catch(Exception ex) {
				out.println(ComLib.getExceptionMsg(ex, "back"));
			...
			try {
				rst2 = TreeList.getExamBeans(id_group, arrTerm_id[0], arrTerm_id[1], aligns, id_subject);
			} catch(Exception ex) {
				out.println(ComLib.getExceptionMsg(ex, "back"));
...
<head>
	...
	<script type="text/javascript">
		...
		%>
					d.add('<%=rst2[i].getId_node()%>','<%=String.valueOf(rst2[i].getId_parentnode())%>','<%=ComLib.toStrHtml(rst2[i].getNode_name())%>','create.jsp?id_node=<%=rst2[i].getId_node()%>&node_rid=<%=rst2[i].getId_parentnode()%>&node_gubun=<%=rst2[i].getNode_gubun()%>','<%=ComLib.toStrHtml(rst2[i].getNode_name())%>','fraMain','<%=img_icon%>','<%=img_icon_open%>');	
...
<BODY id="qman" style="background-image: url(img/bg_tree.gif); background-repeat: no-repeat; over-flow:none;">
