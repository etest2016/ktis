...
<%
  // *************개별시간 연장정보 읽어오기 시작 (by sjh - 2013.10.10)*************
  ...
  // *************개별시간 연장정보 읽어오기 종료 (by sjh - 2013.10.10)*************
...
<%
  // Step 1 : 시험의 기본 설정 값을 읽어온다
	...
	// *************개별시간 연장일경우 개별설정시간 정보 읽어오기 시작 (by sjh - 2013.10.10)*************
		try {
...
<%
    // *************응시자별로 제한시간 연장 기능 추가 시작 (시험연장 시간이 있을 경우 반영한다) (by sjh - 2013.10.10)*************
...
<%
  ...
  if (yn_sametime.equalsIgnoreCase("Y")) { // 동시평가
   ...
   %>   
   <Script type="text/javascript">
	   alert("평가 입장시간 이전입니다. 평가입장시간을 확인하시기 바랍니다.");
	   top.window.close();
   </Script>	
   <%
	   if(true) return;
   ...
   %>
   <Script type="text/javascript">
	   alert("평가 시작시간 이전입니다. 평가시작시간을 확인하시기 바랍니다.");
  ...
  // 3-3 : 시험지가 생성되지 않았을 경우
	if (!(setcount > 0)) {
   %>
   <script type="text/javascript">
	   alert("시험지가 생성되지 않았습니다.");
	   top.window.close();	
   </script>
   <%
	   if(true) return;
...
<%
  ...
  if (hasAnswer == false)
  {
	...
	// *************답안지가 추가 되었으므로 연장 처리가 완료 되었음을 저장하는 부분 시작 (by sjh - 2013.10.10)*************
	if(lngExtTime > 0) {
	...
	// *************답안지가 추가 되었으므로 연장 처리가 완료 되었음을 저장하는 부분 종료*************


  } else { // 새로 입장했을경우 처리
   
	  // *************시험진행중 시간 연장 처리가 되었으므로 시간을 연장작업 시작 (by sjh - 2013.10.10)*************
	  if(ExtYN.equals("N") && lngExtTime > 0) {
	  ...
	  // *************시험진행중 시간 연장 처리가 되었으므로 시간을 연장작업 종료*************
...
function CloseUsingX()
{
  if (log_option == "Y" && ExamEndYN == "N" && document.all.guide.style.display == "none")
  {
    ...
    EvtLog("20");
    myalert("평가중 X 버튼을 이용하여 평가화면을 벗어나는 것은 부정행위로 간주합니다.\n\n상기 사항을 기록하고 운영자에게 통보하였습니다.");
  }
}
...
function Resize()
{
  if (log_option == "Y")
  {
    ...
    Hide();
    EvtLog("11");
    myalert("평가중 창크기를 변경하는 것은 부정행위로 간주합니다.\n\n상기 사항을 기록하고 운영자에게 통보하였습니다.");
...
function CheckMove()
{
  if (log_option == "Y")
  {
    ...
    if (isWindowMoved) {
      ...
      Hide();
      EvtLog("12");
      myalert("평가중 창위치를 변경하는 것은 부정행위로 간주합니다.\n\n상기 사항을 기록하고 운영자에게 통보하였습니다.");
      ExamEndYN = "Y"; // CloseUsingX 를 무시하기 위함임
      parent.close();
    }
  }
...
function LostFocus()
{
  ...
  if (log_option == "Y" && ExamEndYN == "N" && document.all.guide.style.display == "none")
  {
    ...
    if (keyCodeTabEsc == 9) { // Alt+Tab
      EvtLog("22");
      myalert("평가중 평가화면을 벗어나는 것은 부정행위로 간주합니다.\n\n상기 사항을 기록하고 운영자에게 통보하였습니다.");
      keyCodeTabEsc = 0;
...
function chkkeydown(e)
{
  if (log_option == "Y" && ExamEndYN == "N" && document.all.guide.style.display == "none")
  {
    ...
    if (kc == 93 || kc == 113 || kc >= 116 && kc <= 123) {
      // myalert("평가중 누르신 키는 사용하실 수 없습니다 ...");
      event.keyCode = 0;
    ...
    // 주의항목
      ...
      Hide();
      EvtLog("51");
      myalert("평가중 Windows키를 사용하실 수 없습니다\n\n문제가 보이지 않으면 상단 우측에 있는 새로고침을 누르세요.");
    }
    else if (kc == 93) {
      // Context menu
      myalert("평가중 컨텍스트 메뉴키를 사용하실 수 없습니다\n\n문제가 보이지 않으면 상단 우측에 있는 새로고침을 누르세요.");
      event.keyCode = 0;
      return false;
      ...
      EvtLog("53");
      Hide();
      myalert("평가중 F1키를 사용하실 수 없습니다\n\n도움말창을 닫으세요\n\n문제가 보이지 않으면 상단 우측에 있는 새로고침을 누르세요.");
    }
    else if (kc == 114) {
      // F3 (IE 검색창)
      myalert("평가중 F3키를 사용하실 수 없습니다");
      event.keyCode = 0;
      return false;
							...
							<TR>
								...
								<TD align="center">
									...
									<% if( ! yn_sametime.equalsIgnoreCase("Y")) { %>
												<li>평가에 응시할 준비가 되셨으면 평가시작 버튼을 클릭하여 평가에 응시하십시요.</li>
									...
									<% } else { %>
												<li>대기시간이 끝나면 자동으로 평가가 시작 됩니다.</li> 
												<hr>
												<li>평가시작 대기시간</li>
					...
					<TABLE id="table" cellpadding="0" cellspacing="0" border="0">					
						...
						<TR>
							<TD id="B">
								...
								<div class="button" align="left"><img alt="답안 재전송" onclick="javascript:retrySubmit();" src="<%=imagesDir%>/bt_retry.gif" style="cursor: pointer;"></div>
							</TD>		
						</TR>				
					</TABLE>					
				</div>
