...
public class IngInwon
{
	...
	public static IngInwonBean[] getBeans(String start_date, String end_date) throws QmTmException
    {
        ...
        try
        {
            ...
            throw new QmTmException("1검색일자 내 시험정보 읽어오는 중 오류가 발생되었습니다. " + ex.getMessage());
        }
        finally {
	...
	public static IngInwonBean getInwons(String id_exam, int id_auth_type) throws QmTmException 
	{
		...
		if(id_auth_type == 0) { // 로그인
			...
			sql.append("       (Select count(userid) as totcnt From exam_receipt Where id_exam = ?) as tot_inwon, ");
		}
		sql.append("       (Select count(userid) From exam_ans Where id_exam  = ? and userid <> 'tman@@2008' and yn_end = 'Y') AS y, ");
		sql.append("       (Select count(userid) From exam_ans Where id_exam = ? and userid <> 'tman@@2008' and yn_end = 'N') AS n ");
		sql.append("From exam_m ");
	    ...
	    try {
			...
			throw new QmTmException("2검색일자 내 시험 인원정보 읽어오는 중 오류가 발생되었습니다. " + ex.getMessage());
		} finally { 
			if (rst != null) try { rst.close(); } catch (SQLException ex) {}
			if (stm != null) try { stm.close(); } catch (SQLException ex) {}
			if (cnn != null) try { cnn.close(); } catch (SQLException ex) {}
	...
	public static IngInwonBean[] getInwonRes(String start_date, String end_date) throws QmTmException
    {
        ...
        try
        {
            ...
            throw new QmTmException("2.검색일자 내 시험 응시현황정보 읽어오는 중 오류가 발생되었습니다. " + ex.getMessage());			
        }
        finally {
            if (rst != null) try { rst.close(); } catch (SQLException ex) {}
            if (stm != null) try { stm.close(); } catch (SQLException ex) {}
	...
	private static IngInwonBean makeInwonRes(ResultSet rst) throws QmTmException
    {
		try {
            ...
            throw new QmTmException("2.검색일자 내 시험정보 읽어오는 중 오류가 발생되었습니다. " + ex.getMessage());
        }
    }
