...
public class QListGrid
{
    public QListGrid() {
    }
		
	/* 모듈 선택시 쿼리 */
	public static QListGridBean[] getUBeans(String id_subject, String id_chapter, int posStart, int count) throws QmTmException
    {
		...
		sql.append("       convert(varchar(10), regdate, 120) regdate, correct_pct, make_cnt, q ");
		sql.append("From ( ");
		...
		sql.append("			 convert(varchar(10), a.regdate, 120) regdate, a.correct_pct, c.make_cnt, a.q, ");
        ...
        try
        {
            ...
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다. [QListGrid.getUBeans] " + ex.getMessage());
        }
        finally {
	...
	private static QListGridBean makeLists (ResultSet rst) throws QmTmException
    {
		try {
            ...
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다. [QListGrid.makeLists] " + ex.getMessage());
        }
	}

	public static int getCnt(String id_subject, String id_chapter) throws QmTmException
	{
		Connection cnn = null; PreparedStatement stm = null; ResultSet rst = null; 
		StringBuffer sql = new StringBuffer();
		...
		try {
            ...
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다.[QListGrid.getCnt] " + ex.getMessage());
        } finally {
            if (rst != null) try { rst.close(); } catch (SQLException ex) {}
            if (stm != null) try { stm.close(); } catch (SQLException ex) {}
            if (cnn != null) try { cnn.close(); } catch (SQLException ex) {}
	...
	public static QListGridBean[] getExcelBeans(String id_qs) throws QmTmException
    {
		...
		sql.append("Select a.id_q, a.ca, a.id_qtype, case when a.id_qtype = 1 then 'OX형' when a.id_qtype = 2 then '선다형' when a.id_qtype = 3 then '복수답안형' ");
		sql.append("  	   when a.id_qtype = 4 then '단답형'  when a.id_qtype = 5 then '논술형' end qtype, a.excount, a.cacount, b.difficulty, ");
		sql.append("	   convert(varchar(10), a.regdate, 120) regdate, a.correct_pct, c.make_cnt, a.q, a.ex1, a.ex2, a.ex3, a.ex4, ");
		sql.append("       a.ex5, a.explain, a.userid, a.id_valid_type, f.course, d.q_subject, e.chapter, a.find_kwd, g.q_use  ");
		sql.append("From q as a inner join  r_difficulty as b on a.id_difficulty1 = b.id_difficulty ");
		sql.append("     inner join make_q as c on a.id_q = c.id_q ");
		...
		sql.append("     inner join r_q_use as g on a.id_q_use = g.id_q_use ");
        ...
        try
        {
            ...
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다. [QListGrid.getExcelBeans] " + ex.getMessage());
        }
        finally {
            if (rst != null) try { rst.close(); } catch (SQLException ex) {}
            if (stm != null) try { stm.close(); } catch (SQLException ex) {}
	...
	private static QListGridBean makeExcel (ResultSet rst) throws QmTmException
    {
		try {
			...
			bean.setCorrect_pct(rst.getString(9));			
			bean.setCnt(rst.getString(10));
			bean.setQ(rst.getString(11));
			bean.setEx1(rst.getString(12));
			bean.setEx2(rst.getString(13));
			bean.setEx3(rst.getString(14));
			bean.setEx4(rst.getString(15));
			bean.setEx5(rst.getString(16));
			bean.setExplain(rst.getString(17));
			bean.setUserid(rst.getString(18));
			bean.setId_valid_type(rst.getString(19));
			bean.setCourse(rst.getString(20));
			bean.setQ_subject(rst.getString(21));
			bean.setChapter(rst.getString(22));
			bean.setFind_kwd(rst.getString(23));
			bean.setQ_use(rst.getString(24));
			return bean;
        } catch (SQLException ex) {
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다. [QListGrid.makeExcel] " + ex.getMessage());
        }
	}
