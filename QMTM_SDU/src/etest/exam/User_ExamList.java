...
public class User_ExamList
{
    ...
    public static User_ExamListBean[] getBeans(String userid, String id_course, String id_subject) throws QmTmException
    {
		...
		sql.append("WHERE b.userid = ? and a.id_course = ? and a.id_subject = ? AND b.yn_valid = 'Y' AND b.id_course = a.id_course ");
		...
		sql.append("WHERE b.userid = ? and a.id_course = ? and a.id_subject = ? AND b.yn_pay = 'Y' AND b.id_exam = a.id_exam AND ");
		...
		sql.append("WHERE a.id_auth_type = '0' and a.id_course = ? and a.id_subject = ? AND a.yn_enable = 'Y' ");
        ...
        try
        {
            ...
            throw new QmTmException("개설되어진 시험정보를 읽어오는 중 오류가 발생되었습니다. [User_ExamList.getBeans] " +ex.getMessage());
        }
        finally {
    ...
    private static User_ExamListBean makeBeans (ResultSet rst) throws QmTmException
    {
        try {
            ...
            throw new QmTmException("개설되어진 시험정보를 읽어오는 중 오류가 발생되었습니다. [User_ExamList.makeBeans] " +ex.getMessage());
        }
    }
}
