...
public class ManagerSubjUtil
{
	...
	public static ManagerQBean getQBean(String userid, String id_course) throws QmTmException 
	{
		...
		sql.append("From q_worker_subj a inner join q_subject b ");
		sql.append("     on a.id_subject = b.id_q_subject and a.userid = ? and b.id_course = ? ");  
		sql.append("     limit 1 "); 
	    ...
	    try {
			...
			throw new QmTmException("담당자 문제은행 권한정보를 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [ManagerSubjUtil.getQBean]");
		} finally { 
			if (rst != null) try { rst.close(); } catch (SQLException ex) {}
			if (stm != null) try { stm.close(); } catch (SQLException ex) {}
			if (cnn != null) try { cnn.close(); } catch (SQLException ex) {}
