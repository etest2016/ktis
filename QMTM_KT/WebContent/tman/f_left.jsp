...
<%
	...
	if(!id_subject.equals("")) {
		...
		if(userid.equals("qmtm")) {
			try {
				rst = TreeList.getTBeans(userid, years, id_category, aligns, id_subject);
			} catch(Exception ex) {
				out.println(ComLib.getExceptionMsg(ex, "back"));
			...
			try {
				rst = TreeList.getMgrBeans(userid, years, id_category, aligns, id_subject);
			} catch(Exception ex) {
				out.println(ComLib.getExceptionMsg(ex, "back"));
	...
	if(userid.equals("qmtm")) {
		...
		try {
			rst3 = TreeList.getAdmCourseBeans(userid, years, id_category, aligns);		
		} catch(Exception ex) {
			out.println(ComLib.getExceptionMsg(ex, "back"));
		...
		try {
			rst3 = TreeList.getAdmCourseBeans2(userid, years, id_category, aligns);		
		} catch(Exception ex) {
			out.println(ComLib.getExceptionMsg(ex, "back"));
	...
	<script type="text/javascript">
		...
		<%
				for (int i = 0; i < rst.length; i++) { 
					...
					if(rst[i].getNode_gubun().substring(0,1).equals("S")) {
						img_icon = "img/subject.gif";
						img_icon_open = "img/subject_open.gif";
					} else if(rst[i].getNode_gubun().substring(0,1).equals("E")) {
						img_icon = "img/exam.gif";
						img_icon_open = "";
					} else {
		...
		%>
					d.add('<%=rst[i].getId_node()%>','<%=String.valueOf(rst[i].getId_parentnode())%>','<%=rst[i].getNode_name()%>','create.jsp?id_node=<%=rst[i].getId_node()%>&node_rid=<%=rst[i].getId_parentnode()%>&node_gubun=<%=rst[i].getNode_gubun()%>','','fraMain','<%=img_icon%>','<%=img_icon_open%>');
		<% 
			   }
		%>
...
<div style="position: absolute; top: 115px; left: 40px;">
		...
		<select name="id_subject" onChange="cat_select2(this.value);" width="40">
			<% if(rst3 == null) { %>
			<option value="">개설된 과정이 없습니다.</option>
			<% } else { %>
			    <option value="">과정을 선택해 주세요.</option>
				<% for(int i=0; i<rst3.length; i++) { %>				
				<option value="<%=rst3[i].getId_node()%>" <% if(id_subject.equals(rst3[i].getId_node())) { %> selected <% } %>><%if(rst3[i].getNode_name().length() > 15) { %><%=rst3[i].getNode_name().substring(0,15)+".."%><% } else { %><%=rst3[i].getNode_name()%><% } %></option>
				<% } %>
			<% } %>
			</select>
		</form>
