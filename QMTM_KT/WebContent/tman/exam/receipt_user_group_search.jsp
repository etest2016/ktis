			...
			if(frmx.userid.length == undefined) {
			 ...
			 if(k == 0) {
				...
				if (st == true) {
					...
					document.frmdata.action = 'receipt_user_group_search_ok.jsp';
	...
	<div id="contents">
		...
		<table border="0" cellpadding ="0" cellspacing="0" id="tableD" width="100%">			
			...
			<tr>
				...
				<select name="jikmu">
				<% if(jikmuRst == null) { %>
				<option value="">등록직무없음</option>
				<%
					} else {
				%>
				<option value="" <% if(jikmu.equals("")) { %>selected<% } %>>직무선택</option>
				<%
						for(int k=0; k<jikmuRst.length; k++) {
				%>
				<option value="<%=jikmuRst[k].getJikmu()%>" <% if(jikmu.equals(jikmuRst[k].getJikmu())) { %>selected<% } %>><%=jikmuRst[k].getJikmu()%></option>				
				<%
						}
					}
				%>
		...
		<table border="0" cellpadding ="0" cellspacing="0" id="tableA" width="100%">
			...
			<tr id="td" align="center">
				<td colspan="7" class="blank">대상자를 검색하세요.</td>			
			</tr>
			<% 
				} else {
					if(rst == null) {
			%>
			<tr id="td" align="center">
				<td colspan="7" class="blank">검색되어진 대상자가 없습니다.</td>			
			</tr>
			<%
					} else {
			...
			<tr id="td" align="center">				
				<td width="5%"><input type="checkbox" name="userid" class="form2" value="<%=rst[i].getUserid()%>"></td>
