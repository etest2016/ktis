...
<head>
	...
	function exam_edits() {
		window.open("exam_edit.jsp?id_exam=<%=id_exam%>","edit","width=750, height=640, scrollbars=no, top=0, left=0");
    }
	function paper_insert() {
		window.open("paper_index.jsp?id_exam=<%=id_exam%>","inserts","width=900, height=650, scrollbars=yes, top=0, left=0");
	}

	function paper_view(nr_set) {
		window.open("paper_type.jsp?id_exam=<%=id_exam%>&nr_set="+nr_set,"paper_view","width=1000, height=700, scrollbars=yes, top=0, left=0");
	}

	function answer_view() {
		window.open("../answer/answer_main.jsp?id_exam=<%=id_exam%>&id_course=<%=id_course%>","answer_view","width=1200, height=750, top=0, left=0, scrollbars=yes");
	}

	function answer_view2() {
		window.open("../answer/answer_main_20080829.jsp?id_exam=<%=id_exam%>","answer_view","fullscreen, top=0, left=0");
	}

	function static_view() {
		window.open("../static/static_main.jsp?id_exam=<%=id_exam%>","static_view","fullscreen, scrollbars=yes, top=0, left=0");
	}

	function exam_preview() {
		window.open("web_preview.jsp?id_exam=<%=id_exam%>","web_preview","width=400, height=250, scrollbars=no, top=0, left=0");
	}
	</script>
...
<BODY id="tman">
	...
	<div id="main">
		...
		<div id="mainTop">
			<div class="title">시험지 관리 <span>선택된 시험 편집 및 시험지 생성 그리고 응시자의 답안지를 관리합니다</span></div>
			<div class="location">시험관리 > <%=course_name%> ><span> <%=exam_title%> </span></div>
		</div>
