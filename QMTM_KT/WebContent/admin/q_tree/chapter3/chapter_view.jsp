...
//   모 듈 명 : 세번째 단원확인
//   설    명 : 문제은행 세번째 단원확인 팝업 페이지
//   테 이 블 : q_chapter3
//   자바파일 : qmtm.admin.qman.Chapter3QmanBean, qmtm.admin.qman.Chapter3QmanUtil
//   작 성 일 : 2008-04-03
//   작 성 자 : 이테스트 석준호
//   수 정 일 : 
//   수 정 자 : 
...
<script language="javascript">
    // 단원 수정
	function cpt_edit() {
		location.href="chapter_edit.jsp?id_subject=<%=id_subject%>&id_chapter1=<%=id_chapter1%>&id_chapter2=<%=id_chapter2%>&id_q_chapter=<%=id_q_chapter%>";
	}
	
	//--  단원 삭제체크
    function cpt_delete()  {
       var st = confirm("*주의* 단원정보를 삭제하시겠습니까? \n\n삭제 전 해당 단원에 포함된 문항 및 하위단원을 \n먼저 삭제하셔야 합니다." );
       if (st == true) {
           document.location = "chapter_delete.jsp?id_subject=<%=id_subject%>&id_chapter1=<%=id_chapter1%>&id_chapter2=<%=id_chapter2%>&id_q_chapter=<%=id_q_chapter%>";
       }
    }
</script>
