...
<%
		...
		String temps = request.getParameter("temps2");
		...
		String[] arrFileName = temps.split(";");
		...
		{			
			String filesList = arrFileName[file_cnt].replace("\\","/");
			String s = (String)e.nextElement();
			q = q.replaceAll("file:///"+filesList, strSaveUrl+"/"+new String(s.getBytes("iso8859-1"), "euc-kr"));
			file_cnt = file_cnt + 1;
		}
		...
		q = q.replace("</BODY>","");
		q = q.replace("</HTML>","");
		q = q.replace("<BODY><!--StartFragment-->","");
					...
					if(diff_sel) {
						...
						if(explain_sel) {							
							...
							if(hint_sel) {
								...
								if(a == (qcount - 1)) {
									arrQs[a] = q.substring(q.lastIndexOf(q1+q2+(a+1)+q4)).replace(q1+q2+(a+1)+q4,"");
								} else {
									arrQs[a] = q.substring(q.lastIndexOf(q1+q2+(a+1)+q4),q.indexOf(q1+q2+(a+2)+q4)).replace(q1+q2+(a+1)+q4,"");
								}
								arrExplain[a] = "";
								arrHint[a] = "";
								arrDifficulty[a] = "";
	...
	for(int k=0; k < qcount; k++) {
		...
		arrCa[k] = arrCa[k].trim().replaceAll("<BR>","");
		...
		bean.setExplain(ComLib.removeBR(ComLib.nullChk(arrExplain[k])));
		...
		bean.setHint(ComLib.removeBR(ComLib.nullChk(arrHint[k])));
...
%>
<script language="javascript">
	alert("문항 일괄등록 작업중에 오류가 발생했습니다.\n\n문제 및 보기, 정답, 해설등에 패턴 내용이 들어가 있을경우 \n\n패턴이 정상적으로 적용이 되지 않아서 일괄등록이 되지 않습니다.\n\n패턴적용 후 반드시 작업결과화면에서 문항별로 패턴적용이 되었는지 확인해주시기 바랍니다.\n\n\n계속해서 문제가 발생할 경우 관리자에게 문의하시기 바랍니다.");
	history.back();
</script>
<%
		if(true) return;
