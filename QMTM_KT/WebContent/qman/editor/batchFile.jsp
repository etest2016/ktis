...
{	
	...
	if(caNotChk == "N") {
		alert(caNotStr+"문항 정답이 잘못되었습니다.\n\n작업결과 화면에서 오류 문항을 확인해서 수정한 후\n\n패턴적용을 다시 진행하시기 바랍니다.");
		return;
	}
...
function ca_check(num, ca, cacount, qtype) {
	...
	if(ca.length == 0) {
		alert((num+1)+"번 문항 정답이 누락되었습니다.\n\n작업파일에서 해당 문항 정답을 입력하시고\n\n패턴적용을 진행해주시기 바랍니다.");
	...
	if(qtype < 4) { 
		if(cacount == 1) {
			if(ca.length > 1) {
				...
				alert((num+1)+"번 문항 정답이 잘못된 형식입니다.\n\n작업파일에서 해당 문항 정답에 숫자만 다시 입력하시고\n\n패턴적용을 진행해주시기 바랍니다.");			
	...
	if(q3 == "[0-9]") { 
		...
		for(var a=0; a<qcounts; a++) {
			...
			if(arrQ[a].indexOf(e1+e2+e3.substring(4,5)+e4) == -1) { // 선다형 5번 보기가 없을 경우
				...
				arrQ[a] = text_convert(arrQ[a].substring(arrQ[a].lastIndexOf(ca_ptn)));
...
function workResultAll() 
{
	...
	strArea += "<table border='0' width='100%'>";
	strArea += "<tr>";
	strArea += "<td><a href='javascript:workResult(0)'>[ 한문항씩 보기 ]</a>&nbsp;&nbsp;<a href='javascript:workResultAll()'>[ 전체문제보기 ]</a>&nbsp;&nbsp;<a href='javascript:workResultNot()'>[ 정답오류보기 ]</a></td>";
	strArea += "</tr>";
	strArea += "</table>";
...
function workResultNot() 
{
	...
	strArea += "</table>";	
		...
		strArea += "<td colspan='2' align='center'>정답오류 문항이 없습니다.</td>";
		strArea += "</tr>";	
		strArea += "</table>";
		strArea += "</td>";	
		strArea += "</tr>";
...
<body onLoad="movieLayout('div_0');" id="popup2">
	...
	<div id="contents" style="height: 420px;">
		...
		<table border="0" width="100%" cellspacing="0" cellpadding="0">
			<tr>
				<td>* 문항수 : <input type="text" name="qcounts" size="3"> 문항&nbsp;&nbsp;<b><a href="../../sampleFile.zip">[일괄등록샘플다운]</a> <a href="../../guide.zip">[문항저작가이드다운]</a></b></td>
				<td align="right">
		...
		<div style="display:none;" id="div_0">
			...
			<table border="0" width="100%" cellspacing="0" cellpadding="0">
				<tr>
					<td align="left">&nbsp;&nbsp;▶ 작업파일선택&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" checked name="input_text" value="Y">&nbsp;<font color=red><b>텍스트문서로 일괄등록(일괄등록 파일에 이미지가 없을경우 체크)</b></font></td>
				</tr>
				<tr>
					<td valign="top">
