			...
			<td width="500" align="left" valign="top" bgcolor="ffffff">
				...
				<div id="id_q">
								...
								<table width="100%" border="0">
									...
									<tr>
										...
										<table width="100%" border="0" style="display:none;" id="id_correct<%=i+1%>">
											...
											<tr>
												<td><textarea cols="65" rows="5">※ 설정한 정답갯수대로 정답을 입력하세요.<BR>
					단답형 문제의 경우 비슷한 정답이 여러 개 있는 경우가 많습니다. 정답으로 인정될 수 있는 단어를 모두 입력하시면 채점할 때 자동으로 정답처리 됩니다.<BR> 
					예를 들어 정답이 '인간' 이라고 하면 '사람' 이라는 단어도 정답으로 인정된다고 생각되면 모두 입력하세요.</textarea></td>
								...
								<tr>
									<td><input type="checkbox" name="yn_single_line" value="Y" <% if(yn_single_line.equals("Y")) { %>checked<% } %>>&nbsp;단답형 입력 박스 1줄에 표시여부</td>
								</tr>
								<tr>
									<td><input type="checkbox" name="yn_caorder" value="Y" <% if(yn_caorder.equals("Y")) { %>checked<% } %>>&nbsp;정답순서 있음&nbsp;&nbsp; <input type="checkbox" name="yn_case" value="Y" <% if(yn_case.equals("Y")) { %>checked<% } %>>&nbsp;대소문자 구분 있음&nbsp;&nbsp;<input type="checkbox" name="yn_blank" value="Y" <% if(yn_blank.equals("Y")) { %>checked<% } %>>&nbsp;띄어쓰기 구분 있음</td>
								</tr>
								<tr>
									<td>
										<textarea cols="65" rows="10">※ 정답순서<BR> 
			정답이 2개 이상인 문제에서 정답을 순서대로 입력해야 정답 인정할 것인지 순서와 관계없이 해당 단어가 들어 있으면 정답으로 인정할 것인지 설정하는 옵션입니다.<BR>
			예)  정답이 '국가, 기업' 이고, '정답 순서 있음' 으로 할 경우  '기업, 국가' 로 답안을 제출하면 틀린 답안으로 채점이 됩니다.<BR><BR><BR>※ 대소문자 구분<BR>
			영문답안일 경우 대소문자를 구분할 것인지 설정하는 옵션입니다.<BR>
			예)  정답이 'eTEST' 이고, '대소문자 구분 있음' 으로 할 경우  'etest' 로 답안을 제출하면 틀린 답안으로 채점이 됩니다.<BR><BR><BR>
			※ 띄어쓰기 구분<BR>
			정답의 띄어쓰기를 구분할 것인지 설정하는 옵션입니다.<BR>
