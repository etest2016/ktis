...
<%@ page import="java.io.*, java.util.*, com.tagfree.util.*, org.w3c.tidy.*, org.w3c.dom.*, qmtm.*, qmtm.common.*, qmtm.qman.editor.*, qmtm.qman.*, qmtm.admin.etc.EnvUtil" %>

<%!
	private String text_convert(String ca) {
	...
	private String htmlDel(String Word) {		
...
<%	
	...
	{		
		...
		if (input_text == null) { input_text = "N"; } else { input_text = input_text.trim(); }
...
<%
		...
		if(strMimeValue.indexOf("<?xml:namespace prefix = v ns = urn:schemas-microsoft-com:vml />") != -1) {
			...
			pre_str = strMimeValue.substring(0,strMimeValue.indexOf("<?xml:namespace prefix = v ns = urn:schemas-microsoft-com:vml />"));

			strMimeValue = strMimeValue.substring(strMimeValue.indexOf("<?xml:namespace prefix = v ns = urn:schemas-microsoft-com:vml />")); 
		...
		if(q3.equals("[0-9]")) {
			...
			if(arrQ[a].indexOf(e1+e2+e3.substring(2,3)+e4) == -1) { // 선다형 3번 보기가 없을 경우(OX형문제)
				...
				arrQ[a] = arrQ[a].substring(arrQ[a].lastIndexOf(q1+q2+arrMunje_result[a]+q4),arrQ[a].indexOf(ca_ptn)) + text_convert(arrQ[a].substring(arrQ[a].lastIndexOf(ca_ptn)));
...
%>
<script language="javascript">
	alert("2.문항 일괄등록 작업중에 문제가 발생했습니다.\n\n이미지 파일이 원본문서에 포함되어 있을경우\n\n파일 Size가 클경우 업로드가 되지 않는경우가 있습니다.\n\n이미지 파일을 편집한 후 다시 진행해주시기 바랍니다.\n\n\n계속해서 문제가 발생할 경우 관리자에게 문의하시기 바랍니다.");
	window.opener.location.reload();
	window.close();
</script>
<%
