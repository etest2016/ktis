...
<HTML>
 <HEAD>
  ...
  <!--
	...
	function m1_1_pop() {
		window.open("q_qtype.jsp?id_q_subject=<%=names.getId_subject()%>&id_q_chapter=<%=names.getId_chapter1()%>&id_q_chapter2=<%=names.getId_chapter2()%>","QInsert","width=500,height=500,scrollbars=yes");
	}

	function m1_2_pop() {
		window.open("../editor/batchFile.jsp?id_q_subject=<%=names.getId_subject()%>&id_q_chapter=<%=names.getId_chapter1()%>&id_q_chapter2=<%=names.getId_chapter2()%>","BatchInsert","width=700,height=730,scrollbars=yes");	
	}
	...
	function cpt_insert() {
		...
		window.open("../../admin/q_tree/chapter3/chapter_insert.jsp?id_q_chapter=<%=id_q_chapter%>","insert","width=400, height=250, scrollbars=no");
    }

	function cpt_view() {
		subManuClear();

		window.open("../../admin/q_tree/chapter2/chapter_view.jsp?id_subject=<%=names.getId_subject()%>&id_q_subject=<%=id_q_subject%>&id_q_chapter=<%=id_q_chapter%>","view","width=400, height=250, scrollbars=no");
    }
	...
	function q_preview() {
	 ...
	 function q_search() {
		window.open("q_search2.jsp?id_subject=<%=names.getId_subject()%>&id_chapter=<%=id_q_subject%>&id_chapter2=<%=id_q_chapter%>","q_search","width=1000, height=600, scrollbars=yes");
	 }
	 ...
	 var gridQString = "";//we'll save here the last url with query string we used for loading grid (see step 5 for details)
	  ...
	  function init(){
		...
		gridQString = "dyngrid_getXML2.jsp?id_subject=<%=names.getId_subject()%>&id_chapter=<%=id_q_subject%>&id_chapter2=<%=id_q_chapter%>";
