...
<%    
	...
	try {
		names = NameUtil.getChapter4(id_q_chapter);
	} catch(Exception ex) {
...
<HTML>
 <HEAD>
  ...
  <!--
	...
	function m1_1_pop() {
		window.open("q_qtype.jsp?id_q_subject=<%=names.getId_subject()%>&id_q_chapter=<%=names.getId_chapter1()%>&id_q_chapter2=<%=names.getId_chapter2()%>&id_q_chapter3=<%=names.getId_chapter3()%>&id_q_chapter4=<%=names.getId_chapter4()%>","QInsert","width=500,height=500,scrollbars=yes");
	}

	function m1_2_pop() {
		window.open("../editor/batchFile.jsp?id_q_subject=<%=names.getId_subject()%>&id_q_chapter=<%=names.getId_chapter1()%>&id_q_chapter2=<%=names.getId_chapter2()%>&id_q_chapter3=<%=names.getId_chapter3()%>&id_q_chapter4=<%=names.getId_chapter4()%>","BatchInsert","width=700,height=730,scrollbars=yes");	
	}
	...
	function cpt_view() {
		...
		window.open("../../admin/q_tree/chapter4/chapter_view.jsp?id_subject=<%=names.getId_subject()%>&id_chapter1=<%=names.getId_chapter1()%>&id_chapter2=<%=names.getId_chapter2()%>&id_chapter3=<%=names.getId_chapter3()%>&id_q_chapter=<%=id_q_chapter%>","view","width=400, height=250, scrollbars=no");
    }
	...
	function q_preview() {
	 ...
	 function q_search() {
		window.open("q_search4.jsp?id_subject=<%=names.getId_subject()%>&id_chapter=<%=names.getId_chapter1()%>&id_chapter2=<%=names.getId_chapter2()%>&id_chapter3=<%=names.getId_chapter3()%>&id_chapter4=<%=names.getId_chapter4()%>","q_search","width=1000, height=600, scrollbars=yes");
	 }
	 ...
	 var gridQString = "";//we'll save here the last url with query string we used for loading grid (see step 5 for details)
	  ...
	  function init(){
		...
		gridQString = "dyngrid_getXML4.jsp?id_subject=<%=names.getId_subject()%>&id_chapter=<%=names.getId_chapter1()%>&id_chapter2=<%=names.getId_chapter2()%>&id_chapter3=<%=names.getId_chapter3()%>&id_chapter4=<%=names.getId_chapter4()%>";
	...
	<div id="main">
		...
		<table border="0" cellpadding ="0" cellspacing="0">
			<tr>
				...
				<td name='menu3' id='menu3' onMouseOver='menuColorChange(3)' onMouseOut='menuColorBack(3)' onclick="cpt_view();"><img src="../../images/bt6_qman_3.gif" style="cursor: hand;"></td>
