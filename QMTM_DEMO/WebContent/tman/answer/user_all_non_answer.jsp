...
<%
	...
	try {
		beans = PaperNonExport.getAllBeans(id_exam);
	} catch(Exception ex) {
...
<table border="0" width="980" bgcolor="#CCCCCC" cellpadding="3" cellspacing="1">
	<tr bgcolor="#FFFFFF">
		...
		<td width="930" valign="middle">&nbsp;&nbsp;<b><%=beans[i].getQ()%></b> (문제코드 : <b><%=beans[i].getId_q()%></b>, 배점 : <b><%=beans[i].getAllotting()%></b>)</td>
	</tr>
</table>
...
<%
		...
		try {
			beans2 = PaperNonExport.getAllBeans2(id_exam, beans[i].getId_q());
		} catch(Exception ex) {
