...
<%
	...
	try {
		rst2 = ManagerQUtil.getBean(userid, id_course);
	} catch(Exception ex) {
		out.println(ComLib.getExceptionMsg(ex, "close"));
...
<HEAD>
	...
	<script language="javascript">
		function edits() {
			location.href="q_manager_edit.jsp?userid=<%=userid%>&id_course=<%=id_course%>";
		}
		
		//--  삭제체크
		function deletes()  {
		   var st = confirm("*주의* 담당과목을 삭제하시겠습니까?" );
		   if (st == true) {
			   document.location = "q_manager_delete.jsp?userid=<%=userid%>&id_course=<%=id_course%>";
		   }
		}
	</script>
</HEAD>
...
<BODY ID="popup2">
	...
	<div id="top">
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tr>
				<Td id="left"></td>
				<Td id="mid"><div class="title">과목정보 조회 <span>담당 과목정보 조회 및 수정, 삭제</span></div></td>
				<Td id="right"></td>
			</tr>
		</table>
	</div>
	...
	<div id="contents" style="text-align: center;">
		<table cellpadding="0" cellspacing="0" border="0" id="tableD" width="100%">
			<tr>
				<td id="left" width="110">과목명&nbsp;</td>
				<td><%=rst.getCourse()%></td>
			</tr>
			<tr>
				<td id="left">문제편집권한&nbsp;</td>
				<td><%=rst2.getPt_q_edit()%></td>
			</tr>
			<tr>
				<td id="left">문제삭제권한&nbsp;</td>
				<td><%=rst2.getPt_q_delete()%></td>
			</tr>
			<tr>
