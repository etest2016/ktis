...
public class LogFind
{
    ...
    public static LogFindBean[] getBeans(String id_exam) throws QmTmException {
		...
		sql.append("Select userid, nullif(answers, '') answer, nr_set ");
		...
		try {
			...
			throw new QmTmException("응시자 답안정보 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [LogFind.getBeans]");
		} finally {
	...
	private static LogFindBean makeBeans(ResultSet rst) throws QmTmException {
		try {
			...
			throw new QmTmException("응시자 답안정보 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [LogFind.makeBeans]");
		}
	}
	...
	public static LogFindBean[] getNonBeans(String id_exam, String userid, int nr_set) throws QmTmException {
		...
		sql.append("Select b.nr_q, nullif(a.userans1, '') userans1, nullif(a.userans2, '') userans2, ");
		sql.append("       nullif(a.userans3, '') userans3, nullif(a.userans4, '') userans4, nullif(a.userans5, '') userans5  ");
		...
		try {
			...
			throw new QmTmException("응시자 논술형 답안정보 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [LogFind.getNonBeans]");
		} finally {
			if (rst != null) 	try { rst.close(); } catch (SQLException ex) {}
			if (stm != null) 	try { stm.close(); } catch (SQLException ex) {}
			if (cnn != null) 	try { cnn.close(); } catch (SQLException ex) {}
	...
	private static LogFindBean makeNonBeans(ResultSet rst) throws QmTmException {
		try {
			...
			throw new QmTmException("응시자 논술형 답안정보 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [LogFind.makeNonBeans]");
		}
	}
}