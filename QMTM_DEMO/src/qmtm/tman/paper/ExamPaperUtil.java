...
public class ExamPaperUtil
{
	...
	public static ExamOrderQBean[] getOrderQ(String id_exam, String randomtypes) throws QmTmException
	{
		...
		sql.append("Select id_q, id_ref, id_qtype, allotting, excount From q_paper Where id_exam = ? ");
	    ...
	    try {
			...
			throw new QmTmException("시험지 정보 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [ExamPaperUtil.getOrderQ]");
		} finally { 
	...
	private static ExamOrderQBean makeOrderQ (ResultSet rst) throws QmTmException
    {
		...
		try {
            ...
            throw new QmTmException("시험지 정보 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [ExamPaperUtil.makeOrderQ]");
        }
    }
	...
	public static ExamOrderQBean[] getOrderQs(String id_exam, int orders) throws QmTmException
	{
		...
		sql.append("Select id_q, id_subject, id_chapter, id_ref, id_qtype, allotting, excount, ch_qs, ch_score ");
	    ...
	    try {
			...
			throw new QmTmException("시험지 정보 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [ExamPaperUtil.getOrderQs]");
		} finally { 
			if (rst != null) try { rst.close(); } catch (SQLException ex) {}
			if (stm != null) try { stm.close(); } catch (SQLException ex) {}
			if (cnn != null) try { cnn.close(); } catch (SQLException ex) {}
	...
	public static void insert(String id_exam, int nr_set, int nr_q, int id_q, String ex_order, double allotting, int page, int q_no1, int q_no2) throws QmTmException
    {
        ...
        try
        {
            ...
            throw new QmTmException("시험지 정보 등록하는 중 인터넷 연결상태가 좋지 않습니다. [ExamPaperUtil.getOrderQ]");
        }
        finally {
            if (stm != null) try { stm.close(); } catch (SQLException ex) {}
            if (cnn != null) try { cnn.close(); } catch (SQLException ex) {}
