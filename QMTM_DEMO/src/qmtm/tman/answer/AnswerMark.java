...
public class AnswerMark
{
	...
	public static AnswerMarkBean[] getBeans(String id_exam) throws QmTmException
    {
		...
		sql.append("Select distinct c.id_q, c.cacount, c.id_qtype, c.q, c.yn_caorder, c.yn_case, c.yn_blank ");
        ...
        try
        {
            ...
            throw new QmTmException("시험지 문항 정보 읽어오는중 인터넷 연결상태가 좋지 않습니다. [AnswerMark.getBeans]");
        }
        finally {
	...
	public static AnswerMarkBean[] getBeans2(String id_exam) throws QmTmException
    {
        ...
        try
        {
            ...
            throw new QmTmException("시험지 문항 정보 읽어오는중 인터넷 연결상태가 좋지 않습니다. [AnswerMark.getBeans2]");
        }
        finally {
            if (rst != null) try { rst.close(); } catch (SQLException ex) {}
            if (stm != null) try { stm.close(); } catch (SQLException ex) {}
    ...
    private static AnswerMarkBean makeBeans (ResultSet rst) throws QmTmException
    {
		try {
            ...
            throw new QmTmException("시험지 문항 정보 읽어오는중 인터넷 연결상태가 좋지 않습니다. [AnswerMark.makeBeans]");
        }
    }

	public static ImsiAnswerMarkBean getScoreInwon(String id_exam, long id_q) throws QmTmException
    {
        Connection cnn = null; PreparedStatement stm = null; ResultSet rst = null;
		...
		//sql.append("       (select isnull(count(userid),0) from imsi_exam_ans_result where id_exam = ? and id_q = ? and score_yn = 'N' and answer is not null and answer <> '') n_score, "); 
		//sql.append("       (select isnull(count(userid),0) from imsi_exam_ans_result where id_exam = ? and id_q = ? and score_yn = 'Y' and answer is not null and answer <> '') y_score ");	
		sql.append("       (select nullif(count(userid),0) from imsi_exam_ans_result where id_exam = ? and id_q = ? and score_yn = 'N' and answer is not null and answer <> '') n_score, "); 
		sql.append("       (select nullif(count(userid),0) from imsi_exam_ans_result where id_exam = ? and id_q = ? and score_yn = 'Y' and answer is not null and answer <> '') y_score ");	
        ...
        try
        {
            ...
            throw new QmTmException("단답형 채점인원 정보 읽어오는중 인터넷 연결상태가 좋지 않습니다. [AnswerMark.getScoreInwon]");
        }
        finally {
            if (rst != null) try { rst.close(); } catch (SQLException ex) {}
            if (stm != null) try { stm.close(); } catch (SQLException ex) {}
	...
	public static ImsiAnswerMarkBean getAllScoreInwon(String id_exam) throws QmTmException
    {
		...
		sql.append("       (select nullif(count(userid),0) from imsi_exam_ans_result where id_exam = ? and score_yn = 'N' and answer is not null and answer <> '') n_score, "); 
		sql.append("       (select nullif(count(userid),0) from imsi_exam_ans_result where id_exam = ? and score_yn = 'Y' and answer is not null and answer <> '') y_score, ");	
	...
	private static ImsiAnswerMarkBean makeAllScoreInwonBean (ResultSet rst) throws QmTmException
    {
		try {
            ...
            throw new QmTmException("단답형 채점인원 정보 읽어오는중 인터넷 연결상태가 좋지 않습니다. [AnswerMark.makeAllScoreInwon]");
        }
    }
	...
	private static ImsiAnswerMarkBean makeScoreInwonBean (ResultSet rst) throws QmTmException
    {
		try {
            ...
            throw new QmTmException("단답형 채점인원 정보 읽어오는중 인터넷 연결상태가 좋지 않습니다. [AnswerMark.makeScoreInwon]");
        }
    }
}