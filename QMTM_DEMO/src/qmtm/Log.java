...
public class Log
{
    ...
    public static LogAnswerBean[] getBeans(String id_exam, String userid) throws QmTmException {
		...
		sb.append("Select nullif(b.ex_order,'') ex_order, c.id_qtype, b.nr_q ");
		...
		try {
			...
			throw new QmTmException("응시자 로그정보 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [Log.getBeans]");
		} finally {
	...
	private static LogAnswerBean makeBeans(ResultSet rst) throws QmTmException {
		try {
			...
			throw new QmTmException("응시자 로그정보 읽어오는 중 인터넷 연결상태가 좋지 않습니다. [Log.makeBeans]");
		}
	}
