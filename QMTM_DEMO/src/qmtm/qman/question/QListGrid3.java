...
public class QListGrid3
{
    public QListGrid3() {
    }
		
	/* 모듈 선택시 쿼리 */
	public static QListGridBean[] getUBeans(String id_subject, String id_chapter, String id_chapter2, String id_chapter3, int posStart, int count) throws QmTmException
    {
		...
		sql.append("Where a.id_subject = ? and a.id_chapter = ? and a.id_chapter2 = ? and a.id_chapter3 = ? and a.id_chapter4 = '-1' ");
        ...
        try
        {
            ...
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다. [QListGrid3.getUBeans] " + ex.getMessage());
        }
        finally {
	...
	private static QListGridBean makeLists (ResultSet rst) throws QmTmException
    {
		try {
            ...
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다. [QListGrid3.makeLists] " + ex.getMessage());
        }
	}

	public static int getCnt(String id_subject, String id_chapter, String id_chapter2, String id_chapter3) throws QmTmException
	{
		Connection cnn = null; PreparedStatement stm = null; ResultSet rst = null; 
		StringBuffer sql = new StringBuffer();
		...
		try {
            ...
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다.[QListGrid3.getCnt] " + ex.getMessage());
        } finally {
            if (rst != null) try { rst.close(); } catch (SQLException ex) {}
            if (stm != null) try { stm.close(); } catch (SQLException ex) {}
            if (cnn != null) try { cnn.close(); } catch (SQLException ex) {}
	...
	public static QListGridBean[] getExcelBeans(String id_qs) throws QmTmException
    {
        ...
        try
        {
            ...
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다. [QListGrid3.getExcelBeans] " + ex.getMessage());
        }
        finally {
            if (rst != null) try { rst.close(); } catch (SQLException ex) {}
            if (stm != null) try { stm.close(); } catch (SQLException ex) {}
	...
	private static QListGridBean makeExcel (ResultSet rst) throws QmTmException
    {
		try {
            ...
            throw new QmTmException("등록된 문제정보를 읽어오는 중 오류가 발생되었습니다. [QListGrid3.makeExcel] " + ex.getMessage());
        }
	}
