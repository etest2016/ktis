...
public class QBatchUtil
{
	...
	public static void insert(QBatchBean bean) throws QmTmException
    {
		...
		sql.append("              excount, cacount, id_difficulty1, q, ex1, ex2, ex3, ex4, ex5, ca, explain, hint, userid, input_text) "); 
        sql.append("VALUES (?, ?, ?, ?, ?, ?, ?, ");
		sql.append("        ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ");
        ...
        try
        {
			...
			stm.setString(19, bean.getHint());
			stm.setString(20, bean.getUserid());
			stm.setString(21, bean.getInput_text());

			stm.execute();
        }
        catch (SQLException ex) {
            throw new QmTmException("문제 일괄 등록하는 중 인터넷 연결상태가 좋지 않습니다. [QBatchUtil.insert]");        
        }
        finally {
